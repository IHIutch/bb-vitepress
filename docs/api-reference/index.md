...
All requests require a Bearer token in the `Authorization` header. See [Authentication & OAuth](../guides/authentication-and-oauth.md).
...
Search endpoints return FHIR Bundles. Read endpoints (with an ID) return a single resource. See [Fetching Patient Data](../guides/fetching-patient-data.md) for details on reading responses.
