...
Before the legal text, here's what the Terms of Service mean in practice:

- **You must be a HIPAA Covered Entity or Business Associate.** Individual developers or general-purpose apps cannot use DPC.
- **Data is for treatment purposes only.** You may not use DPC data for marketing, research (without separate authorization), or any purpose outside of treating the patient.
- **You must display the CMS attribution notice** in your application: _"This product uses the Data at the Point of Care API but is not endorsed or certified by the Centers for Medicare & Medicaid Services or the U.S. Department of Health and Human Services."_
- **Your software must hold a recognized security certification** (ONC, HITRUST, EHNAC, SOC 2, or ISO 27001/27017/27018).
- **Keep your credentials secret.** Client tokens, keys, and passwords may not be shared or embedded in open-source projects.
- **CMS can revoke access** if you violate these terms or if CMS determines it necessary to prevent abuse.
- **The API is provided "as-is"** with no warranty of availability or accuracy.

::: info
This summary is for convenience only. The full legal text below governs your use of DPC.
:::
